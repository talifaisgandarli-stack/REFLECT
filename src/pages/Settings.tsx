...
function InvitationsSettings() {
  ...
  const invite = useMutation({
    mutationFn: async () => {
      ...
      // D-1 — backend writes the audit row (logAudit 'invitation.created'),
      // no client-side dup write here.
      const inviteUrl = (body as { invite_url?: string }).invite_url;
      const tokenValue = (body as { token?: string }).token;
      // Fallback in case invite_url is missing for any reason — build it
      // the same way the backend would on this host.
