...
**REQ-TASK-01b** Create modal: when both Başlama and Bitmə tarixi are set, **Müddət auto-fills** with the working-day span (Mon–Fri), still editable; flipping Vahid gün↔saat converts the value (8h/workday). Editing a subtask's title/deadline/assignee(s) or deleting it is done by **opening that subtask** in the same card-detail modal (each task — parent or child — is its own card, per Trello). Hard delete is admin-only via RLS `tasks_admin_delete`.
