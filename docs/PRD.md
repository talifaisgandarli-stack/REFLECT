...
- Added REQ-DASH-08 — Finance widget removed from dashboard per product decision; lives only in Maliyyə Mərkəzi
- US-DASH-03 (finance snapshot on dashboard) is now superseded; should be removed in next user-stories revision
- Detailed specs for presence + focus already documented in §10.5 (added in v3.8 addendum)
...
**REQ-DASH-01** Admin sees: active projects health, team workload, deadlines ≤14d, 50 latest activity entries (Realtime), MIRAI quick-launch.
...
**REQ-TASK-01** Quick create (title only) + full create modal (title, assignee_ids[], project, start_date, deadline, estimated_duration, duration_unit, risk_buffer_pct, is_expertise_subtask, task_level, parent_task_id). Both the create- and edit-task modals embed a shared **manual subtask builder** (`SubtaskBuilder`) in the same card: the user adds any number of subtask rows, each with its own title, its own **mandatory deadline**, and its own **mandatory assignee(s)**. Subtasks are optional, but a non-empty row must have all three; fully-empty rows are ignored. On submit each subtask is inserted as a linked child (`parent_task_id`, `task_level + 1`). The edit modal also lists existing subtasks (read-only) for context and only shows the builder for top-level tasks (one level deep). Flow: write the parent → assign it → optionally add independently-assigned, deadline-bearing subtasks.

**REQ-TASK-01a** Board UX: there is **one card-detail modal** (Trello-style) — `TaskCommentsModal` — opened by clicking a card body, the ⋯ → "Aç" item, or a subtask row. It edits every field inline (title, status, priority, labels, dates, estimate, assignees, description), shows the subtask checklist, and hosts comments/@mentions — plus the admin-only toggle and hard delete. There is no separate edit form. (Quick task **create** still uses the dedicated TaskCreateModal.) Modals animate in (fade + subtle scale, `modal-pop`; respects `prefers-reduced-motion`) and are centered/viewport-bounded with a pinned header + scrolling body. Across Cədvəl / Gantt / Calendar a subtask shows its parent ("↳ parent title").

**REQ-TASK-01b** Duration is shown as **working days (Mon–Fri) + work hours (8h/day)** computed from Başlama/Bitmə — in the create modal (a readout; auto-fills the editable Müddət, Vahid gün↔saat converts) and as a live readout in the detail modal's date row. **Subtask checklist (detail modal):** children list inside the parent card; clicking a row **expands its details inline** (deadline + assignees via the same inline editors, plus delete) — no separate popup. New subtasks are added via an inline form at the **bottom of the list** ("+ Yarımtapşırıq əlavə et") that requires **title + deadline + at least one assignee** before "Əlavə et" enables (assignees default to the creator; admins can pick anyone). Completed subtasks stay visible (struck-through) because the list includes archived children. Hard delete is admin-only via RLS `tasks_admin_delete`.
...
**REQ-TASK-08** Archive (updated by migration 0069): only **`Cancelled`** auto-archives (`archived_at = now()`, trigger `tasks_auto_archive`) — cancelled has no board column, so it belongs in Arxiv. **`Tamamlandı` no longer auto-archives**: completed tasks stay visible as live cards in the Tamamlandı column (the column shows a real count, not 0). Archiving a done task is explicit — bulk "Arxivlə" or the Arxiv flow — after which it leaves the board and appears behind the column's "+ N daha" expander / in the Arxiv module (Module 5). This supersedes the original "done → hidden" behaviour and design §8.3's "empty Tamamlandı stub" (the column is now populated). Cancelled subtasks still archive, but the board re-fetches archived direct children of visible parents so they stay visible (struck-through) inside the parent checklist with a correct X/Y count.
...
**REQ-FIN-10** Project contract value (migration 0087): `projects.contract_value_net` (ƏDV-siz) + `projects.vat_rate` (default 18), captured on create/edit; gross = net·(1+vat/100). Profit is always computed on the ƏDV-siz figure (VAT is passed to the state, not company revenue). Set on the project itself; the CRM `receivables` model stays the client-side tool (no dual entry).
**REQ-FIN-11** Project payments = installments of the contract value. `incomes.payment_kind` (advance|interim|final) labels the milestone; the project Maliyyə surface calls this "Ödəniş" (not "Gəlir") and shows müqavilə / alınıb / qalıq progress. Per-payment ƏDV (migration 0088): `incomes.vat_included` + `incomes.vat_rate` — optional checkbox in the payment form (auto-ticked when method = Bank köçürməsi, admin can untick); net-profit maths divide only ƏDV-li payments by their rate, cash payments count in full. Subcontractors bound to the project (`outsource_items.project_id`) appear on the project P&L, with paid/committed sourced from the `outsource_payments` milestones (not `outsource_items.paid_at`).
**REQ-FIN-12** Overhead allocation + net profit (Rentabellik, migration 0087): shared overhead per month = active AZN salaries + layihəsiz (general) expenses. `project_overhead_allocations(project_id, period_month, percent, overhead_amount, locked)` stores each project's share; default split is by contract-value share, admin overrides manually. Per-project net profit = payments received (ƏDV-siz) − direct costs (operativ + podratçı) − Σ allocated overhead. Saving snapshots the AZN amount so historical months stay stable when salaries later change. Admin-only.
...
## 10.5 Presence + Focus Mode (PRD v3.8 addendum)

### 10.5.1 Presence (REQ-PRESENCE-*)

**REQ-PRESENCE-01 — Online status visible to all team members**
- Each authenticated user has presence state: `online | away | offline`
- Visible to ALL team members (manager and member roles), not admin-restricted — fosters team awareness
- Updates via Supabase realtime channel; no polling

**REQ-PRESENCE-02 — Status semantics**
- `online`: active session within last 60 seconds (heartbeat)
- `away`: session active but no input for ≥5 min OR tab not focused for ≥3 min
- `offline`: heartbeat lost ≥90 seconds OR explicit logout
- Status displayed as colored dot: green / amber / gray

**REQ-PRESENCE-03 — Current location label**
- When `online`, show which page user is on: "Tapşırıqlarda" / "Layihələrdə" / "Maliyyədə"
- Privacy: page label only, no detail (e.g. not "Aksent Group profilində" — just "Müştərilərdə")
- Helps coordination: "Səbinə Tapşırıqlarda → mən rahat ona text edə bilərəm"

**REQ-PRESENCE-04 — Last seen timestamp**
- For `offline` users: "5 dəq əvvəl", "2 saat əvvəl", "Dünən 14:30", "3 gün əvvəl"
- Uses relative time within 7 days, absolute after
...
### 10.5.2 Focus Mode (REQ-FOCUS-*)

**REQ-FOCUS-01 — Personal opt-in tool**
- Strictly personal — never visible to other team members
- No effect on time-tracking, billing, velocity calculations
- Pure motivational/wellness tool; failure to use carries no penalty

**REQ-FOCUS-02 — Configurable intervals**
- Presets: 25/5, 40/20 (default), 50/10, 90/15
- Custom values: 15-180 min work, 5-30 min break

**REQ-FOCUS-03 — Chameleon mascot growth**
- Mascot displays growth stage 1→4 during work session (egg → baby → juvenile → adult chameleon)
- Visible only to the user themselves
- Session interruption returns mascot to start; no "death" animation (avoid anxiety reinforcement in work tool)

**REQ-FOCUS-04 — Local-first state**
- Session state persists in localStorage; survives page reload
- Optional cloud sync (per-user table `focus_sessions` for personal stats only)
- Stats: sessions per day/week, total focused time, longest streak

**REQ-FOCUS-05 — Notification**
- Browser notification on session/break end (if permission granted)
- Optional sound: chameleon-themed click (designed audio cue, not mimicking real animal voice)
- Mute toggle in settings

**REQ-FOCUS-06 — Dashboard widget placement**
- Compact widget on dashboard (right column, lower bento)
- Click expands to full-screen focus mode (single-task focus surface)
- Hidden state preserved if user dismisses for current session

**Data:** `focus_sessions` (optional cloud persistence) — `user_id, started_at, planned_minutes, completed_at, interrupted, mascot_stage`
**RLS:** user can only read/write their own rows
**Out of scope (v1):** ambient sounds, achievement/streak rewards, multi-device session continuation — defer to v1.5
